...
| agents/tour | runs | confirmés/run | tokens/run | tokens par finding |
|---|---|---|---|---|
| 1 | 22 | 1,5 | 85 k | 55 k |
| 2-3 | 17 | 4,1 | 278 k | 68 k |
| **4-5** | 15 | **9,3** | 553 k | **59 k** |
| 6-8 | 5 | 12,6 | 919 k | 73 k |
| 9+ | 7 | 12,7 | 1 384 k | 109 k |
...
- **Leçon sur *comment reviewer*** réutilisable ailleurs → **promotion par confirmation, en deux temps** (hors dépôt ; créer les fichiers si absents). Format : `- [<dimension>] **<titre>** : <règle actionnable> — *vu sur N runs*`, étiquette obligatoire parmi `spec correctness security design tests perf harness`.
  1. **Chercher d'abord dans `senior-review-lessons-candidates.md`** une leçon décrivant le même mécanisme. Si elle existe, la **promouvoir** dans `senior-review-lessons.md` en passant à *vu sur 2 runs*, et la retirer des candidates.
  2. Sinon, chercher dans `senior-review-lessons.md` : si elle y est déjà, incrémenter N. Sinon, écrire la leçon dans **`senior-review-lessons-candidates.md`**, jamais directement dans la mémoire chargée.
  Le fichier chargé ne contient donc que ce qui s'est produit au moins deux fois. C'est la même exigence que pour un finding : ce qui n'a été vu qu'une fois est un candidat, pas une règle. **Plafond 40** sur la mémoire chargée : au-delà, élaguer les plus petits N d'abord, puis les plus anciens. Les candidates n'ont pas de plafond puisqu'elles ne coûtent rien. **Anonymisation obligatoire** : ce fichier peut être publié (repo public) — jamais de nom client / projet / vendor / branche réels ; généraliser (« un projet réel »). Ces leçons sont rechargées à l'Étape 1.8 de chaque run — c'est ce qui ferme la boucle d'apprentissage.
...
- **Ligne de run** — une par revue, TOUJOURS, quel que soit le projet, la branche ou le verdict → `~/.claude/skill-memory/senior-review-runs.jsonl` (hors dépôt ; créer si absent). **Append-only, une ligne JSON, jamais de relecture ni de réécriture des lignes passées.** Champs **en ASCII sans accent** — ce sont des clés machine, et `jq` ne sait pas lire `.confirmés` sans échappement : `{"date","project","branch","tier","loop","round","loc","raised","confirmed","dropped_gate","refuted_panel","settled","out_of_scope","agents","tokens","verdict"}`. `loc` = lignes modifiées hors fichiers générés, le seul champ qui permette de vérifier après coup si le fan-out était bien dimensionné.. Tous déjà connus à ce stade, aucun jugement du modèle.
  **`tier` est un vocabulaire fermé** : exactement `quick`, `standard`, `deep` ou `confirmation`, et **`loop` est un booléen séparé**. Écrire `standard+loop`, `standard-loop`, `quick-loop` ou `loop` rend le journal inanalysable par tier, ce qui est sa seule raison d'être.
  **Invariant à respecter** : `raised = confirmed + dropped_gate + refuted_panel`. `settled` et `out_of_scope` comptent des entrées de ledger, pas des findings, et n'entrent donc pas dans cette somme. Une ligne qui viole l'invariant est une ligne qu'on ne pourra pas exploiter. C'est la seule mesure de tendance du skill : misses et lessons distillent des règles, ce journal garde le brut. **Ne JAMAIS le charger à l'Étape 1.8** — il n'informe pas une revue, il sert à les analyser toutes ensemble plus tard ; l'ouvrir en revue serait refaire l'erreur de 1.9.0. Pas de plafond : une ligne pèse ~200 octets et n'entre jamais en contexte.
...
1. **Spec coverage** — couvre tout le ticket ? (pas seulement correct). Si le ticket chiffre une gêne, recompter ce chiffre sur le parcours livré ; « comme l'existant » ne répond pas à un ticket qui reproche l'existant.
