...
**skill_version : 1.15.0** (historique : `CHANGELOG.md`). Revue de code de niveau senior, conçue à partir de l'état de l'art académique (LLM-as-judge, vérification, mutation) et des meilleurs outils de revue IA (CodeRabbit, Greptile, Cursor BugBot, GitHub Copilot agentic, Qodo, Snyk).
...
- **Ligne de run** — une par revue, TOUJOURS, quel que soit le projet, la branche ou le verdict → `~/.claude/skill-memory/senior-review-runs.jsonl` (hors dépôt ; créer si absent). **Append-only, une ligne JSON, jamais de relecture ni de réécriture des lignes passées.** Champs **en ASCII sans accent** — ce sont des clés machine, et `jq` ne sait pas lire `.confirmés` sans échappement : `{"date","project","branch","tier","loop","round","raised","confirmed","dropped_gate","refuted_panel","settled","out_of_scope","agents","tokens","verdict"}`. Tous déjà connus à ce stade, aucun jugement du modèle.
