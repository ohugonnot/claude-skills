...
- Sécurité : password hasher `auto`, voters pour l'authz complexe, rate limiter Symfony (3 policies dispo — `fixed_window`/`sliding_window`/`token_bucket`, `fixed_window` la plus simple à raisonner — par IP, fallback défensif `getClientIp() ?? 'unknown'`). Réponses **neutres** sur les chemins de rejet (anti-énumération) : même flash, même redirect que le nominal — et formulation **conditionnelle** (« si votre demande est valide, vous recevrez… ») pour rester vrai sans révéler le rejet. Injecter les **interfaces** (`RateLimiterFactoryInterface`), pas les classes concrètes.
...
- Commandes console : `KernelTestCase::runCommand()` (approche moderne recommandée) ou `CommandTester` (legacy) ; antidater un `createdAt` sans setter via le composant `symfony/clock` (`MockClock`/`ClockSensitiveTrait`, restauration auto) plutôt que `ReflectionProperty` (ne casse pas si le setter devient privé/final).
